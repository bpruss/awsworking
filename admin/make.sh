...
bdm=[{\"DeviceName\":\"/dev/xvda\",\"Ebs\":{\"VolumeSize\":$adminebsvolumesize}}]
echo bdm=$bdm
...
if test -z "$useeip"; then
	...
	# Bernie Pruss 4/18 convert to --domain vpc
        useeip=$(aws ec2 allocate-address --domain vpc --output text --query 'PublicIp')
fi
echo useeip=$useeip

echo get v_ip_alloc_id
# 4/17/2016 Bernie Pruss get the allocation id of the ip address: 
v_ip_alloc_id=$(aws ec2 describe-addresses --filters Name=public-ip,Values=$useeip --output text --query 'Addresses[*].AllocationId')
...
aws ec2 associate-address --instance-id $instance_id --allocation-id $v_ip_alloc_id
